...
var reight =document.querySelector('.reight');
var carousel =document.querySelector('.carousel');

carousel.addEventListener('mouseover',function(){
    left.style.display = 'block';
    reight.style.display = 'block';
    clearInterval(timer);

})
carousel.addEventListener('mouseout',function(){
    left.style.display = 'none';
    reight.style.display = 'none';
...
var flag = true ;
//点击右侧按钮，图片往右滚动一张
reight.addEventListener('click',function(){
   if(flag){
       flag = false;//关闭节流阀
    if(num == 5){
        ul.style.left = 0 ;
...
left.addEventListener('click',function(){
    if(flag == true){
        ...
        console.log( - num * carousel.offsetWidth + 'px');
